...
因为在 8GB 显存的机器上，生成一首 3 分钟的歌要 12~25 分钟。没人会盯着节点图等这么久。
...
### 曲风是「告诉模型这是什么类型的音乐」，不是替你写好的 prompt

界面把两件事分开了：

| | 是什么 | 你要做的 |
|---|---|---|
| **曲风** | 分类信号：这是 lo-fi 还是爵士 | 点一下选中 |
| **人声** | 纯器乐 / 女声 / 男声 / 和声 | 点一下选中（BGM 默认纯器乐） |
| **你想要什么样的** | 这一首的具体意图 | 用你自己的话写，中文就行 |

提交时系统把三者合成为模型要的三段式描述：

```
Global Metadata: 曲风 + BPM + 调性 + 【你的话】 + 曲风固有情绪 + 制作风格
Vocal Details:   按人声取向生成，带该曲风的典型音色
Arrangement:     该曲风的典型编配
```

**你的话放在情绪位置的最前面** —— 它是这一首的具体意图，应该压过曲风自带的通用情绪，而不是被淹没在后面。

合成结果在「实际发给模型的描述」里**完全可见、可直接改**。改过之后就以你改的为准，点「恢复自动合成」回到跟随曲风。不做黑箱 —— 出了不满意的结果，你得能看见模型到底收到了什么。

**为什么不是往描述框里灌模板**（早期版本的做法）：那样把「类型」和「意图」混成一件事，你得去改别人写的段落，换个曲风就丢掉自己写的东西，而且曲风不再是可独立切换的维度。现在换曲风、换人声，你写的话都原样保留。

> **一处未验证的地方**：曲风描述是英文（官方示例都是英文，这些音乐术语在训练数据里英文最密集），而你的话原样保留、不翻译。MiniMax 是国内模型，中英混排大概率能吃，但我没做过对照实验。如果发现中文那段没起作用，就在预览框里把它改成英文再试。
