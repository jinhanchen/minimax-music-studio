...
本机实测（RTX 5070 Laptop 8GB + int8 模型）：
...
| 1 分钟 | 1,500 | 14 ~ 24 分钟 |
| 3 分钟 | 4,500 | 40 ~ 72 分钟 |
| 6 分钟（上限） | 9,000 | 80 ~ 143 分钟 |

**为什么这么慢**：耗时的大头不是扩散采样（30 步只要 13 秒），而是文本编码器。`MiniMaxMusic3AR` 是自回归模型，**步数 = 音乐秒数 × 25**，逐帧生成声学条件序列。所以：

- 调低「扩散步数」几乎省不了时间 —— 它只影响那 13 秒
- **唯一能显著减少等待的办法是缩短时长**
- 首次生成额外花约 76 秒加载模型，之后常驻内存
